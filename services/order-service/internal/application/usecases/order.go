...
import (
	...
	"github.com/yourorg/order/internal/domain"
)

type Repo interface { Create(ctx context.Context, o *domain.Order) error }

type Service struct{ repo Repo; nc *nats.Conn }
...
func (s *Service) CreateOrder(ctx context.Context, userID string, items []domain.OrderItem) (string, error) {
	...
	if err := s.repo.Create(ctx, o); err != nil { return "", err }
	evt := map[string]interface{}{"order_id": id, "user_id": userID, "items": items}
	if payload, _ := json.Marshal(evt); s.nc != nil { _ = s.nc.Publish("order.created", payload) }
